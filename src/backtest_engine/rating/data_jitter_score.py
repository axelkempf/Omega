    ...
    *,
    ...
    for tf in timeframes:
        ...
        atr_array = np.nan_to_num(
            np.asarray(atr_series.to_numpy(dtype=float)),
            nan=0.0,
            posinf=0.0,
            neginf=0.0,
        )
        ...
        base_df_bid = next(
            (
                df
                for (t, s), df in base_preloaded_data.items()
                if t == tf and s.lower() == "bid"
            ),
            None,
        )
        if base_df_bid is not None:
            for i in range(min(5, n)):
                o_val = (
                    float(base_df_bid["Open"].iloc[i])
                    if i < len(base_df_bid)
                    else np.nan
                )
                h_val = (
                    float(base_df_bid["High"].iloc[i])
                    if i < len(base_df_bid)
                    else np.nan
                )
                l_val = (
                    float(base_df_bid["Low"].iloc[i])
                    if i < len(base_df_bid)
                    else np.nan
                )
                c_val = (
                    float(base_df_bid["Close"].iloc[i])
                    if i < len(base_df_bid)
                    else np.nan
                )
        ...
        for candle_type in ("bid", "ask"):
            ...
            if candle_type.lower() == "bid":
                ...
                for i in range(min(5, len(df))):
                    ...
                    min_price_ok = (
                        final_open[i] >= min_price
                        and final_close[i] >= min_price
                        and final_high[i] >= min_price
                        and final_low[i] >= min_price
                    )
                    ...
                    if not all_ok:
                        ...
                        if not high_ok:
                            issues.append(
                                f"High={final_high[i]:.5f} < max(O,C)={max(final_open[i], final_close[i]):.5f}"
                            )
                        if not low_ok:
                            issues.append(
                                f"Low={final_low[i]:.5f} > min(O,C)={min(final_open[i], final_close[i]):.5f}"
                            )
                        if not low_high_ok:
                            issues.append(
                                f"Low={final_low[i]:.5f} > High={final_high[i]:.5f}"
                            )
                        if not min_price_ok:
                            issues.append(
                                f"min(OHLC)={min(final_open[i], final_close[i], final_high[i], final_low[i]):.5f} < min_price={min_price}"
                            )
