...
def run_benchmark(num_runs: int = 3) -> dict[str, Any]:
    ...
    for i in range(num_runs):
        ...
        print(f"  {i+1:<3} {result.time_seconds:>10.3f} {result.peak_memory_mb:>15.1f} {result.total_trades:>8}")
    ...
    print(f"  {'Avg Time (s)':<23} {py_avg_time:>12.3f} {rust_avg_time:>12.3f} {speedup_avg:>11.2f}x")
    print(f"  {'Best Time (s)':<23} {py_min_time:>12.3f} {rust_min_time:>12.3f} {speedup_best:>11.2f}x")
    print(f"  {'Peak RAM (MB)':<23} {py_avg_mem:>12.1f} {rust_avg_mem:>12.1f} {mem_diff:>+11.1f}")
    print(f"  {'Trades':<23} {py_results[0].total_trades:>12} {rust_results[0].total_trades:>12} {'✅ match' if py_results[0].total_trades == rust_results[0].total_trades else '❌ diff':>12}")
    ...
    balance_tolerance = max(py_results[0].final_balance, rust_results[0].final_balance) * 0.001
    balance_match = abs(py_results[0].final_balance - rust_results[0].final_balance) < balance_tolerance
    
    print(f"  Trades match:        {'✅' if trades_match else '❌'} (Python: {py_results[0].total_trades}, Rust: {rust_results[0].total_trades})")
    print(f"  Balance match (<0.1%): {'✅' if balance_match else '❌'} (Python: {py_results[0].final_balance:.2f}, Rust: {rust_results[0].final_balance:.2f})")
    ...
    if all_pass:
        ...
        if mem_diff < 0:
            print(f"✅ Rust uses {abs(mem_diff):.1f}MB less RAM ({abs(mem_pct):.1f}% reduction)")
        else:
            print(f"⚠️  Rust uses {mem_diff:.1f}MB more RAM ({mem_pct:.1f}% overhead - FFI costs)")
    else:
