...
from backtest_engine.core.event_engine import EventEngine, get_active_backend, is_rust_available
...
@pytest.fixture
def multi_candle_data(deterministic_candles: tuple[List[Candle], List[Candle]]) -> Dict[str, Dict[str, List[Candle]]]:
...
class TestEventEngineParity:
    ...
    def test_same_signals_processed(
        ...
        python_executor.process_signal = MagicMock(side_effect=lambda s: python_signals.append(1))
        ...
        rust_executor.process_signal = MagicMock(side_effect=lambda s: rust_signals.append(1))
        ...
        assert len(python_signals) == len(rust_signals), (
        ...
        assert python_executor.process_signal.call_count == rust_executor.process_signal.call_count, (
    ...
    def test_portfolio_updates_count_match(
        ...
        assert python_update_count == rust_update_count, (
    ...
    def test_bars_processed_match(
        ...
        assert python_eval_count[0] == rust_eval_count[0], (
        ...
        if rust_engine.last_stats is not None:
            assert rust_engine.last_stats.bars_processed == python_eval_count[0], (
...
class TestEventEngineRobustness:
    ...
    def test_progress_callback_invoked(
        ...
        assert len(python_progress_calls) == len(rust_progress_calls), (
        ...
        if python_progress_calls:
            assert python_progress_calls[-1][0] == python_progress_calls[-1][1], (
                "Python final progress should be complete"
            )
        if rust_progress_calls:
            assert rust_progress_calls[-1][0] == rust_progress_calls[-1][1], (
